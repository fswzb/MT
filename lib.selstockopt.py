#选股条件
#1.5天35%，5天35%的最高点的前一天至前六天的5日平均换手率大于等于6%
#2.5日EMA价格大于等于前天收盘价的93%。
#3.最近30日涨停次数大于等于3次
#4.股价最高点出现在5个交易日内
#5.系统期望值0.45R
#lib.selstockopt
...
def findcandidate(guci,_enddate,target=2,incr=0.35,duration=5,_EMA=True,howlong=60,_enableprint=False,turnrate=0.06,_begindate='20130101'):
    ...
    stocks = {}
    for s in guci:
        _shis = DataAPI.MktEqudGet(beginDate=_begindate,endDate=_enddate,secID=s,isOpen=1,field=['tradeDate','closePrice','lowestPrice','highestPrice','openPrice','turnoverRate','accumAdjFactor'],pandas='1')
...
def findsinglecandidate(s,_enddate,_shis,target=2,incr=0.35,duration=5,_EMA=True,howlong=60,_enableprint=False,turnrate=0.06,_Divhis=[]):
    global golden8,golden618,golden5,_ma5,_ma10,_ma20
    value = []
    if iscandidate(s,_shis,target,incr,duration,_EMA,howlong,turnrate,_Divhis):
        #check if history max
        history = DataAPI.MktEqudAdjGet(beginDate='19910101',endDate=_enddate,secID=s,isOpen=1,field=['highestPrice','secShortName'],pandas='1')
        _hmax = history['highestPrice'].max()
        _ratio = _shis['closePrice'].iloc[-1]/_hmax-1.
...
def iscandidate(s,_shis,target=2,incr=0.35,duration=5,_EMA=True,howlong=60,turnrate=0.06,_Divhis=[]):
    global golden8,golden618,golden5,_ma5,_ma10,_ma20
    count = len(_shis)
    if(count< howlong):#to caculdate the MA20 of 30 days need at lease 50 days transaction and also we ignore new stock
        return False
    _closePrice = _shis['closePrice'][count-20:count]*_shis['accumAdjFactor'][count-20:count]
    _lowestPrice = _shis['lowestPrice'][count-30:count]*_shis['accumAdjFactor'][count-30:count]
    _highestPrice = _shis['highestPrice'][count-30:count]*_shis['accumAdjFactor'][count-30:count]
    ...
    and (_closep < end):
        ...
        _macloseprice = _shis['closePrice'][-30:]*_shis['accumAdjFactor'][-30:]
        if _EMA:
            MA30l = talib.EMA(_macloseprice.values,timeperiod=30)
            MA20l = talib.EMA(_macloseprice.values,timeperiod=20)
            MA10l = talib.EMA(_macloseprice.values,timeperiod=10)
            MA5l = talib.EMA(_macloseprice.values,timeperiod=5)
            _ma5 = MA5l[-1]
            _ma10 = MA10l[-1]
            _ma20 = MA20l[-1]
        else:
            MA30l = talib.MA(_macloseprice.values,timeperiod=30)
            MA20l = talib.MA(_macloseprice.values,timeperiod=20)
            MA10l = talib.MA(_macloseprice.values,timeperiod=10)
            MA5l = talib.MA(_macloseprice.values,timeperiod=5)
...
for s in uni:
    _shis = DataAPI.MktEqudGet(beginDate='20160101',endDate=end_d,secID=s,isOpen=1,field=['closePrice','lowestPrice','highestPrice','openPrice','turnoverRate','tradeDate','accumAdjFactor'],pandas='1')
