...
def startsimulate(_continueday,_end,_benchmark,_universe,_capital_base,_initialize,_handle_data,_refresh_rate,_freq):
    ...
    if g_EMA:
        data.to_excel('龙回头模拟交易S%s-%s-EMA-%d.xlsx' %(start,_end,g_targetprice),header=indexs)  
    else:
        data.to_excel('龙回头模拟交易S%s-%s-%d.xlsx' %(start,_end,g_targetprice),header=indexs)  
    cansfiltered = {}
